...
function mostrarCaptcha() {
    ...
    for (var imagem = 1; imagem <= 9; imagem++) {
        ...
        if (imagemSorteada == 1) {
            if(imagemCorreta == 0){
                imagemCorreta++     
            imagens.innerHTML += `<img src="imgs/captcha/${imagemSorteada}.png" style="height: 70px; width: 70px;" onclick ="verificarRobo()">`;
            }else {
                imagem--
            }
        } else {
            imagens.innerHTML += `<img src="imgs/captcha/${imagemSorteada}.png" style="height: 70px; width: 70px;" onclick ="errarVerificacao()">`;
        }
        if (imagem % 3 == 0) {
